...
export function handleSurplus(
    ...
    // --- PRIORITY GUARD LAYER: EXECUTE TRUE EMPLOYER MATCHING ---
    if (alive1 && rrspRoom1 > 0 && options?.p1MatchRate && options.p1MatchRate > 0 && options?.p1Tier && options.p1Tier > 0) {
        let maxEmployerCap1 = person1.inc * options.p1MatchRate;
        let requiredEmployeeDeposit1 = maxEmployerCap1 / options.p1Tier;
        let actEmployeePortionP1 = Math.min(Math.max(0, remaining), requiredEmployeeDeposit1, rrspRoom1);

        if (actEmployeePortionP1 > 0) {
            let actEmpPortionP1 = Math.min(rrspRoom1 - actEmployeePortionP1, actEmployeePortionP1 * options.p1Tier, maxEmployerCap1);
            if (options.inflowsReference?.p1) options.inflowsReference.p1.gross += actEmpPortionP1;
            if (options.matchTracking) {
                options.matchTracking.p1MatchAdded = actEmpPortionP1;
                options.matchTracking.totalMatch1 = actEmployeePortionP1 + actEmpPortionP1;
            }
            person1.rrsp += (actEmployeePortionP1 + actEmpPortionP1);
            rrspRoom1 -= (actEmployeePortionP1 + actEmpPortionP1);
            remaining -= actEmployeePortionP1;
            annualContributed.p1.rrsp += (actEmployeePortionP1 + actEmpPortionP1);
            annualContributed.shared.rrsp += (actEmployeePortionP1 + actEmpPortionP1);
            if (flowLog) flowLog.contributions.p1.rrsp = (flowLog.contributions.p1.rrsp || 0) + (actEmployeePortionP1 + actEmpPortionP1);
        }
    }

    if (alive2 && rrspRoom2 > 0 && options?.p2MatchRate && options.p2MatchRate > 0 && options?.p2Tier && options.p2Tier > 0 && remaining > 0) {
        let maxEmployerCap2 = person2.inc * options.p2MatchRate;
        let requiredEmployeeDeposit2 = maxEmployerCap2 / options.p2Tier;
        let actEmployeePortionP2 = Math.min(remaining, requiredEmployeeDeposit2, rrspRoom2);

        if (actEmployeePortionP2 > 0) {
            let actEmpPortionP2 = Math.min(rrspRoom2 - actEmployeePortionP2, actEmployeePortionP2 * options.p2Tier, maxEmployerCap2);
            if (options.inflowsReference?.p2) options.inflowsReference.p2.gross += actEmpPortionP2;
            if (options.matchTracking) {
                options.matchTracking.p2MatchAdded = actEmpPortionP2;
                options.matchTracking.totalMatch2 = actEmployeePortionP2 + actEmpPortionP2;
            }
            person2.rrsp += (actEmployeePortionP2 + actEmpPortionP2);
            rrspRoom2 -= (actEmployeePortionP2 + actEmpPortionP2);
            remaining -= actEmployeePortionP2;
            annualContributed.p2.rrsp += (actEmployeePortionP2 + actEmpPortionP2);
            annualContributed.shared.rrsp += (actEmployeePortionP2 + actEmpPortionP2);
            if (flowLog) flowLog.contributions.p2.rrsp = (flowLog.contributions.p2.rrsp || 0) + (actEmployeePortionP2 + actEmpPortionP2);
        }
    }
    ...
    for (const acct of accumOrder) {
        ...
        if (acct === 'spending_cash') {
            ...
            const useRealDollars = Boolean(inputs.useRealDollars === true || inputs.todays_dollars === true || inputs.use_real_dollars === true);
            if (!useRealDollars) maxSpendingAllowed *= baseInflation;
        ...
        if (acct === 'tfsa') {
            if (alive1 && tfsaRoom1 > 0) { 
                let allowed = isSplit ? (maxLimits.p1.tfsa === 0 ? Infinity : Math.max(0, maxLimits.p1.tfsa - annualContributed.p1.tfsa)) : (maxLimits.p1.tfsa === 0 ? Infinity : Math.max(0, maxLimits.p1.tfsa - annualContributed.shared.tfsa));
                if (allowed > 0) {
                    let take = Math.min(remaining, tfsaRoom1, allowed); 
                    person1.tfsa += take; remaining -= take; tfsaRoom1 -= take; annualContributed.p1.tfsa += take; annualContributed.shared.tfsa += take;
                    if (flowLog) flowLog.contributions.p1.tfsa = (flowLog.contributions.p1.tfsa || 0) + take;
                }
            }
            if (alive2 && tfsaRoom2 > 0 && remaining > 0) { 
                let allowed = isSplit ? (maxLimits.p2.tfsa === 0 ? Infinity : Math.max(0, maxLimits.p2.tfsa - annualContributed.p2.tfsa)) : (maxLimits.p2.tfsa === 0 ? Infinity : Math.max(0, maxLimits.p2.tfsa - annualContributed.shared.tfsa));
                if (allowed > 0) {
                    let take = Math.min(remaining, tfsaRoom2, allowed); 
                    person2.tfsa += take; remaining -= take; tfsaRoom2 -= take; annualContributed.p2.tfsa += take; annualContributed.shared.tfsa += take;
        ...
        if (acct === 'rrsp') {
            if (alive1 && rrspRoom1 > 0 && !options?.blockRRSPContributionsP1) {
                let allowed = isSplit ? (maxLimits.p1.rrsp === 0 ? Infinity : Math.max(0, maxLimits.p1.rrsp - annualContributed.p1.rrsp)) : (maxLimits.p1.rrsp === 0 ? Infinity : Math.max(0, maxLimits.p1.rrsp - annualContributed.shared.rrsp));
                if (allowed > 0) {
                    let take = Math.min(remaining, rrspRoom1, allowed);
                    person1.rrsp += take; remaining -= take; rrspRoom1 -= take; actualDeductions.p1 += take; annualContributed.p1.rrsp += take; annualContributed.shared.rrsp += take;
            ...
            if (alive2 && rrspRoom2 > 0 && remaining > 0 && !options?.blockRRSPContributionsP2) {
                let allowed = isSplit ? (maxLimits.p2.rrsp === 0 ? Infinity : Math.max(0, maxLimits.p2.rrsp - annualContributed.p2.rrsp)) : (maxLimits.p2.rrsp === 0 ? Infinity : Math.max(0, maxLimits.p2.rrsp - annualContributed.shared.rrsp));
                if (allowed > 0) {
                    let take = Math.min(remaining, rrspRoom2, allowed);
                    person2.rrsp += take; remaining -= take; rrspRoom2 -= take; actualDeductions.p2 += take; annualContributed.p2.rrsp += take; annualContributed.shared.rrsp += take;
        ...
        if (acct === 'fhsa') {
            if (alive1 && fhsaLim1 > 0 && fhsaRooms.p1 > 0) { 
                let allowed = isSplit ? (maxLimits.p1.fhsa === 0 ? Infinity : Math.max(0, maxLimits.p1.fhsa - annualContributed.p1.fhsa)) : (maxLimits.p1.fhsa === 0 ? Infinity : Math.max(0, maxLimits.p1.fhsa - annualContributed.shared.fhsa));
                if (allowed > 0) {
                    let take = Math.min(remaining, fhsaLim1, fhsaRooms.p1, allowed); 
                    person1.fhsa += take; remaining -= take; fhsaLim1 -= take; fhsaRooms.p1 -= take; actualDeductions.p1 += take; annualContributed.p1.fhsa += take; annualContributed.shared.fhsa += take;
            ...
            if (alive2 && fhsaLim2 > 0 && fhsaRooms.p2 > 0 && remaining > 0) { 
                let allowed = isSplit ? (maxLimits.p2.fhsa === 0 ? Infinity : Math.max(0, maxLimits.p2.fhsa - annualContributed.p2.fhsa)) : (maxLimits.p2.fhsa === 0 ? Infinity : Math.max(0, maxLimits.p2.fhsa - annualContributed.shared.fhsa));
                if (allowed > 0) {
                    let take = Math.min(remaining, fhsaLim2, fhsaRooms.p2, allowed); 
                    person2.fhsa += take; remaining -= take; fhsaLim2 -= take; fhsaRooms.p2 -= take; actualDeductions.p2 += take; annualContributed.p2.fhsa += take; annualContributed.shared.fhsa += take;
        ...
        if (acct === 'crypto') {
            if (alive1) {
                let allowed = isSplit ? (maxLimits.p1.crypto === 0 ? cryptoLim : Math.min(cryptoLim, Math.max(0, maxLimits.p1.crypto - annualContributed.p1.crypto))) : (maxLimits.p1.crypto === 0 ? cryptoLim : Math.min(cryptoLim, Math.max(0, maxLimits.p1.crypto - annualContributed.shared.crypto)));
                if (allowed > 0) {
                    let take = Math.min(remaining, allowed);
                    person1.crypto += take; person1.crypto_acb += take; remaining -= take; annualContributed.p1.crypto += take; annualContributed.shared.crypto += take;
                    if (flowLog) flowLog.contributions.p1.crypto = (flowLog.contributions.p1.crypto || 0) + take;
                }
            }
            if (alive2 && remaining > 0) {
                let allowed = isSplit ? (maxLimits.p2.crypto === 0 ? cryptoLim : Math.min(cryptoLim, Math.max(0, maxLimits.p2.crypto - annualContributed.p2.crypto))) : (maxLimits.p2.crypto === 0 ? cryptoLim : Math.min(cryptoLim, Math.max(0, maxLimits.p2.crypto - annualContributed.shared.crypto)));
                if (allowed > 0) {
                    let take = Math.min(remaining, allowed);
                    person2.crypto += take; person2.crypto_acb += take; remaining -= take; annualContributed.p2.crypto += take; annualContributed.shared.crypto += take;
        ...
        if (acct === 'nonreg') {
            if (alive1) { 
                let allowed = isSplit ? (maxLimits.p1.nonreg === 0 ? Infinity : Math.max(0, maxLimits.p1.nonreg - annualContributed.p1.nonreg)) : (maxLimits.p1.nonreg === 0 ? Infinity : Math.max(0, maxLimits.p1.nonreg - annualContributed.shared.nonreg));
                if (allowed > 0) {
                    let take = Math.min(remaining / (alive2 ? 2 : 1), allowed); 
                    person1.nonreg += take; person1.acb += take; remaining -= take; annualContributed.p1.nonreg += take; annualContributed.shared.nonreg += take;
                    if (flowLog) flowLog.contributions.p1.nonreg = (flowLog.contributions.p1.nonreg || 0) + take;
                }
            }
            if (alive2 && remaining > 0) { 
                let allowed = isSplit ? (maxLimits.p2.nonreg === 0 ? Infinity : Math.max(0, maxLimits.p2.nonreg - annualContributed.p2.nonreg)) : (maxLimits.p2.nonreg === 0 ? Infinity : Math.max(0, maxLimits.p2.nonreg - annualContributed.shared.nonreg));
                if (allowed > 0) {
                    let take = Math.min(remaining, allowed); 
                    person2.nonreg += take; person2.acb += take; remaining -= take; annualContributed.p2.nonreg += take; annualContributed.shared.nonreg += take;
        ...
        if (acct === 'cash') {
            if (alive1) { 
                let allowed = isSplit ? (maxLimits.p1.cash === 0 ? Infinity : Math.max(0, maxLimits.p1.cash - annualContributed.p1.cash)) : (maxLimits.p1.cash === 0 ? Infinity : Math.max(0, maxLimits.p1.cash - annualContributed.shared.cash));
                if (allowed > 0) {
                    let take = Math.min(remaining / (alive2 ? 2 : 1), allowed); 
                    person1.cash += take; remaining -= take; annualContributed.p1.cash += take; annualContributed.shared.cash += take;
                    if (flowLog) flowLog.contributions.p1.cash = (flowLog.contributions.p1.cash || 0) + take;
                }
            }
            if (alive2 && remaining > 0) { 
                let allowed = isSplit ? (maxLimits.p2.cash === 0 ? Infinity : Math.max(0, maxLimits.p2.cash - annualContributed.p2.cash)) : (maxLimits.p2.cash === 0 ? Infinity : Math.max(0, maxLimits.p2.cash - annualContributed.shared.cash));
                if (allowed > 0) {
                    let take = Math.min(remaining, allowed); 
                    person2.cash += take; remaining -= take; annualContributed.p2.cash += take; annualContributed.shared.cash += take;
...
export function handleDeficit(
    ...
    const executePull = (p: any, prefix: string, acct: string, maxSlice?: number): number => {
        ...
        if (flowLog) flowLog.withdrawals[`${prefix.toUpperCase()} ${logKey}`] = (flowLog.withdrawals[`${prefix.toUpperCase()} ${logKey}`] || 0) + pullAmount;
        
        if (wdBreakdown) {
            if (!wdBreakdown[prefix]) wdBreakdown[prefix] = {};
            if (!wdBreakdown[prefix][logKey + '_math']) wdBreakdown[prefix][logKey + '_math'] = { wd: 0, tax: 0, acb: 0, gain: 0 };
    ...
    for (const acct of decumOrder) {
        ...
        if (['rrsp', 'rrif_acct', 'lif', 'lirf'].includes(acct) && isBothRetired && age1 < 65 && age2 < 65 && alive1 && alive2) {
            ...
            if (alive1) executePull(person1, 'p1', acct);
            if (alive2 && remainingDeficit > 0) executePull(person2, 'p2', acct);
        }
    }

    if (remainingDeficit > 0 && protectedCash > 0) {
        if (alive1 && person1.cash > 0) { availableHouseholdCash = Infinity; executePull(person1, 'p1', 'cash'); }
        if (alive2 && person2.cash > 0 && remainingDeficit > 0) { availableHouseholdCash = Infinity; executePull(person2, 'p2', 'cash'); }
    }
}