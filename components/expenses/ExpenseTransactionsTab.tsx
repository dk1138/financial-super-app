...
export default function ExpenseTransactionsTab({ transactions, categories, formatCurrency }: Props) {
    ...
    const handleExecuteSplit = async () => {
        ...
        const validSplits = splitItems.map(item => ({
                    ...
                    <table className="table table-hover table-borderless align-middle mb-0">
                        ...
                        <tbody>
                            {sortedTransactions.length === 0 ? (
                                <tr><td colSpan={5} className="text-center py-5 text-muted fst-italic">No transactions match your search.</td></tr>
                            ) : (
                                        ...
                                        <tr key={tx.id} className={`transition-all ${isRowSelected ? 'bg-primary bg-opacity-10' : ''}`}>
                                            ...
                                            <td className="py-3 position-relative">
                                                <div 
                                                    ...
                                                    style={{ width: '140px', height: '28px', userSelect: 'none', backgroundColor: bgColor, color: isUncat ? '' : baseColor, borderColor: isUncat ? '' : baseColor }}
                                            ...
                                            <td className="text-end pe-4 py-3 text-nowrap">
                                                <div className="d-flex align-items-center justify-content-end gap-2">
                                                    ...
                                                    <button 
                                                        className="btn btn-sm btn-link p-0 text-muted hover-text-primary transition-all" 
                    ...
                    <div className="modal-dialog modal-dialog-centered modal-lg position-relative">
                        <div className="modal-content surface-card border border-secondary shadow-lg rounded-4">
                            ...
                            <div className="modal-body p-4">
                                    ...
                                    {splitItems.map((item, idx) => (
                                        <div key={idx} className="d-flex gap-2 align-items-center">
                                            <div className="input-group input-group-sm w-25">
                                                ...
                                                <input 
                                                    type="number" 
                                                    className="form-control bg-body border-secondary fw-bold text-main" 
                                                    value={item.amount} 
                                                    onChange={(e) => {
                                                        const newSplits = [...splitItems];
                                                        newSplits[idx].amount = e.target.value;
                                                        setSplitItems(newSplits);
                                                    }} 
                                                />
                                            </div>
                                            
                                            <select 
                                                className="form-select form-select-sm bg-body border-secondary text-main w-25 fw-bold"
                                                value={item.category}
                                                onChange={(e) => {
                                                    const newSplits = [...splitItems];
                                            ...
                                            <input 
                                                type="text" 
                                                className="form-control form-control-sm bg-body border-secondary text-main flex-grow-1" 
                                                placeholder="Custom Note / Merchant"
                                                value={item.merchant}
                                                onChange={(e) => {
                                                    const newSplits = [...splitItems];
                                                    newSplits[idx].merchant = e.target.value;
                                                    setSplitItems(newSplits);
                                                }}
                                            />
                                            
                                            {splitItems.length > 2 && (
                                                <button className="btn btn-sm btn-outline-danger border-0" onClick={() => setSplitItems(splitItems.filter((_, i) => i !== idx))}>
                                ...
                                <button className="btn btn-sm btn-outline-secondary rounded-pill fw-bold border-dashed w-100 py-2 text-muted" onClick={handleAddSplitRow}>
                                    <i className="bi bi-plus-circle-fill me-2"></i> Add Another Split
                                </button>
                            </div>
