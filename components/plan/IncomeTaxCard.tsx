...
export default function IncomeTaxCard() {
  ...
  const calculatePlayerMetrics = (p: 'p1' | 'p2') => {
      ...
      if (!yData) return { gross: getActiveIncome(p), netIncome: getActiveIncome(p) };
      ...
      // Handle custom paycheque deductions
      const pDeds = (data.deductions || []).filter((d: any) => d.owner === p);
      const dedSum = pDeds.reduce((sum: number, d: any) => sum + ((Number(d.amount) || 0) * (d.freq === 'monthly' ? 12 : 26)), 0);

      // Net Income calculation
      const netIncome = actualGross - match - totalTax - dedSum;

      return { actualGross, netIncome };
  };

  const p1Metrics = calculatePlayerMetrics('p1');
  const p2Metrics = isCouple ? calculatePlayerMetrics('p2') : { actualGross: 0, netIncome: 0 };

  const hhGross = p1Metrics.actualGross + p2Metrics.actualGross;
  const hhNet = p1Metrics.netIncome + p2Metrics.netIncome;
  ...
  const renderTaxBox = (taxDetails: any, p: string) => {
      ...
      // Custom Paycheque Deductions
      const pDeds = (data.deductions || []).filter((d: any) => d.owner === p);
      const dedSum = pDeds.reduce((sum: number, d: any) => sum + ((Number(d.amount) || 0) * (d.freq === 'monthly' ? 12 : 26)), 0);

      // Deductions Logic (Strictly Paycheck Deductions)
      const totalDeductions = match + dedSum;
      ...
      pDeds.forEach((d: any) => {
          const val = (Number(d.amount) || 0) * (d.freq === 'monthly' ? 12 : 26);
          if (val > 0) dedBreakdown.push({label: d.name || 'Deduction', val});
      });
      ...
      // Add the utilized credits BACK into the displayed tax
          ...
          <div className="border border-secondary rounded-4 mt-4 shadow-sm transition-all">
            ...
            <div className="p-3 bg-input d-flex flex-column gap-2 rounded-bottom-4">
              ...
              <div className="border-bottom border-secondary border-opacity-50 pb-2 mb-1">
                  <div className="d-flex justify-content-between align-items-center cursor-pointer transition-all user-select-none hover-opacity-75" onClick={() => toggleDeductions(p)}>
                      <span className={`small fw-medium d-flex align-items-center gap-1 ${showDeductions[p] ? 'text-main' : 'text-muted'}`}>
                          <i className={`bi bi-chevron-${showDeductions[p] ? 'up' : 'down'} small`}></i> Deductions
                      </span>
              ...
              <div className="mt-3 pt-3 border-top border-secondary">
                  ...
                  {/* Standard Net Income */}
                  <div className="d-flex justify-content-between align-items-center mb-2">
                      <span className="text-main fw-bold d-flex align-items-center">
                          Net Income 
                          <InfoBtn align="right" title="Net Income" text="Your Gross Income minus deductions (CPP, EI, and Base Income Tax)." />
                      </span> 
                      <span className="text-main fw-bold fs-6">
                          ${Math.round(actualGross - match - taxDetails.totalTax - dedSum - totalRefundFromCredits).toLocaleString()}
                      </span>
                  </div>
                      ...
                      <div className="mt-1 pt-1">
                          ...
                          >
                              <span className="text-success fw-bold d-flex align-items-center gap-1">
                                  ...
                                  <InfoBtn align="right" title="Spring Tax Refund" text="The estimated lump-sum refund you will receive when you file your taxes, generated by your extra Tax Credits (Age Amount, Disability, Donations, etc.)." />
                              </span> 
                  ...
                  {/* True Annual Net Income */}
                  <div className="d-flex justify-content-between align-items-center pt-2 mt-1 border-top border-secondary border-opacity-25">
                      <span className="text-success fw-bolder d-flex align-items-center text-uppercase ls-1" style={{ fontSize: '0.8rem' }}>
                          True Annual Net Income 
                      </span> 
                      <span className="text-success fw-bolder fs-5">
                          ${Math.round(actualGross - match - taxDetails.totalTax - dedSum).toLocaleString()}
                      </span>
                  </div>
            ...
            <div className="col-12 col-xl-6" key={p}>
              ...
              <div className="card h-100 border-secondary surface-card shadow-none mb-4">
                <div className="card-body p-4">
                  ...
                  {/* NEW PAYCHEQUE DEDUCTIONS SECTION */}
                  <div className="d-flex justify-content-between align-items-center mb-2 mt-4">
                      <h6 className="fw-bold text-muted small text-uppercase ls-1 mb-0">Paycheque Deductions</h6>
                      <button type="button" className="btn btn-sm btn-outline-secondary rounded-pill px-2 py-1 fw-bold" style={{fontSize: '0.75rem'}} onClick={() => addArrayItem('deductions', { owner: p, name: 'LTD / Health', amount: 50, freq: 'bi-weekly' })}>+ Add</button>
                  </div>
                          ...
                          <div className="border border-secondary rounded-4 mb-3 shadow-sm" key={`ded_${realIdx}`}>
                              <div className="bg-secondary bg-opacity-10 border-bottom border-secondary p-2 px-3 d-flex justify-content-between align-items-center rounded-top-4">
                                  <div className="d-flex align-items-center gap-3 w-100">
                                      <div className="bg-danger bg-opacity-25 text-danger rounded-circle d-flex align-items-center justify-content-center flex-shrink-0" style={{width: '32px', height: '32px'}}><i className="bi bi-dash-circle"></i></div>
                                      <input type="text" maxLength={50} className="form-control bg-transparent border-0 text-main fw-bold p-0 shadow-none" value={ded.name || ''} onChange={(e) => updateDed('name', e.target.value)} placeholder="Deduction Name" />
                                  </div>
                                  <button type="button" className="btn btn-sm btn-link text-danger p-0 ms-2 opacity-75 hover-opacity-100 flex-shrink-0" onClick={() => removeArrayItem('deductions', realIdx)}><i className="bi bi-x-lg fs-5"></i></button>
                              </div>
                              <div className="p-3 bg-input rounded-bottom-4">
                                  <div className="row g-3">
                                      <div className="col-12 col-md-6">
                                          <label className="small text-muted mb-1 fw-medium">Amount ($)</label>
                                          <CurrencyInput className="form-control form-control-sm" value={ded.amount} onChange={(val: any) => updateDed('amount', val)} />
                                      </div>
                                      <div className="col-12 col-md-6">
                                          <label className="small text-muted mb-1 fw-medium">Frequency</label>
                                          <select className="form-select form-select-sm bg-input border-secondary text-main" value={ded.freq || 'bi-weekly'} onChange={(e) => updateDed('freq', e.target.value)}>
                  ...
                  <div className="border border-secondary rounded-4 mb-3 shadow-sm mt-3">
                      <div className="bg-secondary bg-opacity-10 border-bottom border-secondary p-2 px-3 d-flex align-items-center gap-3 rounded-top-4">
            ...
            <div className="card-body p-4">
              <div className="row text-center align-items-center">
                <div className="col-md-6 border-end border-primary border-opacity-25 mb-3 mb-md-0">
                  <div className="small fw-bold text-primary text-uppercase ls-1 mb-2">Total Household (Gross Income)</div>
                ...
                <div className="col-md-6">
                  <div className="small fw-bold text-success text-uppercase ls-1 mb-2">
                      ...
                      <InfoBtn align="center" title="Household Net Income" text="Combined Gross Income (excluding employer matches) minus Total Taxes. This includes any estimated tax refunds you generate from your credits." />
                  </div>
